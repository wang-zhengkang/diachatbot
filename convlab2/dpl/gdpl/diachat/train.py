...
from convlab2.dpl.etc.test.evaluate import *
...
if __name__ == '__main__':
    ...
    parser.add_argument("--load_path", type=str, help="path of model to load")
    ...
    for i in range(args.epoch):
        ...
        if i % 5 == 0:
            F1, precise, recall = test(policy_sys)
            logging.info(f"F1: {F1}")
            logging.info(f"precise: {precise}")
            ...
            plt.xlabel('epoch')
            plt.ylabel('F1')
            plt.savefig("convlab2/dpl/gdpl/diachat/log/F1.jpg")
